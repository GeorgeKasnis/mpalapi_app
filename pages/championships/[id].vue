<template>
    <div>
        ...
        <ButtonsBaseBtn class="mx-auto block" v-if="championship.finished_at == null" @click="closeChampionship(championship.id)" name="Close Championship"
        ...
        <transition name="fade-in-out">
            <div class="grid grid-cols-1 gap-8 max-w-2xl mx-auto" v-if="!loading">
                <vue-good-table v-if="!loading" :columns="tableContentResults.columns" :rows="tableContentResults.rows"> </vue-good-table>
            </div>
        </transition>
        ...
        <transition name="fade-in-out">
            <div class="grid grid-cols-1 gap-8 max-w-2xl mx-auto" v-if="!loading">
                <vue-good-table v-if="!loading" :columns="tableContentGames.columns" :rows="tableContentGames.rows"> </vue-good-table>
            </div>
        </transition>
    </div>
</template>
        ...
        async getChampionship(id) {
            await $fetch(`${this.baseUrl}/api/championships/${id}`, {
                ...
                response.results.forEach((element, index) => {
                    this.tableContentResults.rows.push({
                        id: element.id,
                        position: index + 1,
                        name: element.name,
                ...
                response.games.forEach((element) => {
                    this.tableContentGames.rows.push({
                        id: element.id,
